...
float3 LumaSharpenPass(float4 position : SV_Position, float2 tex : TEXCOORD) : SV_Target
{
	...
	// change for VREM 
	float4 mask = tex2Dlod(MaskBuffer, float4(tex, 0, 0));
	float4 depth = tex2Dlod(DepthBuffer, float4(tex, 0, 0));
	int area = mak_value(mask, depth);
	if ( (area == COCKPIT && VREMtarget_area == 1) ||  (area != COCKPIT  && VREMtarget_area == 2)  || VREMtarget_area ==0)
	{
		// end of change
